...
static void
moveAOI(aoi_space* aoi, aoi_object* watcher, aoi_object* marker) {
	if (watcher->id == marker->id) {
		return;
	}
	aoi->cb_moveAOI(aoi->cb_ud, watcher->id, marker->id, marker->pos);
}


aoi_space *
aoi_create(aoi_Alloc alloc,void *alloc_ud,float map_size[3],float view_size[3],enterAOI_Callback cb_enterAOI,leaveAOI_Callback cb_leaveAOI, moveAOI_Callback cb_moveAOI, void *cb_ud) {
...
aoi_space *
aoi_new(float map_size[3],float view_size[3],enterAOI_Callback cb_enterAOI,leaveAOI_Callback cb_leaveAOI, moveAOI_Callback cb_moveAOI,void *cb_ud) {
	return aoi_create(default_alloc,NULL,map_size,view_size,cb_enterAOI,cb_leaveAOI, cb_moveAOI,cb_ud);
}
...
aoi_move(aoi_space *aoi,uint32_t id,float pos[3]) {
	...
	//printf("called aoi_move id: %d pos(%f,%f)\r\n", id, pos[0], pos[1]);
	int i;
	...
	for (i = 0; i < aoi->set1->number; i++) {
		...
		moveAOI(aoi, temp, obj);
		//printf("[MOVE111] moveid:%d id:%d, pos(%f,%f)\r\n",id, temp->id, temp->pos[0], temp->pos[1]);
	}
